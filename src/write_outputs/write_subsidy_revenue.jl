...
function write_subsidy_revenue(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if !isempty(inputs["VRE_STOR"])
        ...
        if !isempty(MIN_CAP_SOLAR)
            dfSubRevenue.SubsidyRevenue[MIN_CAP_SOLAR] .+=
                (value.(EP[:eTotalCap_SOLAR])[MIN_CAP_SOLAR]) .*
                (dual.(EP[:cMinCap_Solar][MIN_CAP_SOLAR])).data
        end
        if !isempty(MIN_CAP_WIND)
            dfSubRevenue.SubsidyRevenue[MIN_CAP_WIND] .+=
                (value.(EP[:eTotalCap_WIND])[MIN_CAP_WIND]) .*
                (dual.(EP[:cMinCap_Wind][MIN_CAP_WIND])).data
        end
        if !isempty(MIN_CAP_STOR)
            dfSubRevenue.SubsidyRevenue[MIN_CAP_STOR] .+=
                (value.(EP[:eTotalCap_STOR])[MIN_CAP_STOR]) .*
                (dual.(EP[:cMinCap_Stor][MIN_CAP_STOR])).data
        end
    end
    dfSubRevenue.SubsidyRevenue[MIN_CAP] .=
        (value.(EP[:eTotalCap])[MIN_CAP]) .* (dual.(EP[:cMinCap][MIN_CAP])).data
    ...
    if (setup["MinCapReq"] >= 1)
        for mincap = 1:inputs["NumberOfMinCapReqs"] # This key only exists if MinCapReq >= 1, so we can't get it at the top outside of this condition.
            MIN_CAP_GEN = ids_with_policy(gen, min_cap, tag = mincap)
            dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN] .=
                dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN] +
                (value.(EP[:eTotalCap][MIN_CAP_GEN])) * (dual.(EP[:cZoneMinCapReq][mincap]))
            if !isempty(inputs["VRE_STOR"])
                ...
                MIN_CAP_GEN_ASYM_DC_DIS =
                    intersect(inputs["VS_ASYM_DC_DISCHARGE"], HAS_MIN_CAP_STOR)
                MIN_CAP_GEN_ASYM_AC_DIS =
                    intersect(inputs["VS_ASYM_AC_DISCHARGE"], HAS_MIN_CAP_STOR)
                ...
                if !isempty(MIN_CAP_GEN_SOLAR)
                    dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN_SOLAR] .+= (
                        ...
                        etainverter.(
                            gen[ids_with_policy(gen, min_cap_solar, tag = mincap)]
                ...
                if !isempty(MIN_CAP_GEN_ASYM_DC_DIS)
                    ...
                    dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN_ASYM_DC_DIS] .+= (
                        (
                            value.(
                                EP[:eTotalCapDischarge_DC][MIN_CAP_GEN_ASYM_DC_DIS].data
                            ) .*
                            etainverter.(
                                gen_VRE_STOR[min_cap_stor.(
                                    gen_VRE_STOR,
                                    tag = mincap,
                                ).==1 .& (gen_VRE_STOR.stor_dc_discharge.==2)]
                            )
                        ) * (dual.(EP[:cZoneMinCapReq][mincap]))
                    )
                end
                if !isempty(MIN_CAP_GEN_ASYM_AC_DIS)
                    dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN_ASYM_AC_DIS] .+= (
                        (value.(EP[:eTotalCapDischarge_AC][MIN_CAP_GEN_ASYM_AC_DIS]).data) * (dual.(EP[:cZoneMinCapReq][mincap]))
                    )
                end
                        ...
                        (
                            ...
                            power_to_energy_dc.(
                                gen_VRE_STOR[(min_cap_stor.(
                                    gen_VRE_STOR,
                                    tag = mincap,
                                ).==1 .& (gen_VRE_STOR.stor_dc_discharge.==1))]
                            ) .*
                            etainverter.(
                                gen_VRE_STOR[(min_cap_stor.(
                                    gen_VRE_STOR,
                        ...
                        (
                            ...
                            power_to_energy_ac.(
                                gen_VRE_STOR[(min_cap_stor.(
                                    gen_VRE_STOR,
                                    tag = mincap,
                                ).==1 .& (gen_VRE_STOR.stor_ac_discharge.==1))]
                            )
                        ) * (dual.(EP[:cZoneMinCapReq][mincap]))
                    )
                end
