...
function write_capacityfactor(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if !isempty(VRE_STOR)
        ...
        if !isempty(SOLAR)
            dfCapacityfactor.AnnualSum[SOLAR] .=
                value.(EP[:vP_SOLAR][SOLAR, :]).data * inputs["omega"] * scale_factor
            dfCapacityfactor.Capacity[SOLAR] .=
                value.(EP[:eTotalCap_SOLAR][SOLAR]).data * scale_factor
        end
        if !isempty(WIND)
            dfCapacityfactor.AnnualSum[WIND] .=
                value.(EP[:vP_WIND][WIND, :]).data * inputs["omega"] * scale_factor
            dfCapacityfactor.Capacity[WIND] .=
                value.(EP[:eTotalCap_WIND][WIND]).data * scale_factor
        end
        if !isempty(SOLAR_WIND)
            dfCapacityfactor.AnnualSum[SOLAR_WIND] .=
                (
                ...
                inputs["omega"] *
                scale_factor
            dfCapacityfactor.Capacity[SOLAR_WIND] .=
                (
    ...
    EXISTING = intersect(
        findall(x -> x >= 1, dfCapacityfactor.AnnualSum),
    ...
    dfCapacityfactor.CapacityFactor[CF_GEN] .=
        (dfCapacityfactor.AnnualSum[CF_GEN] ./ dfCapacityfactor.Capacity[CF_GEN]) /
        sum(inputs["omega"][t] for t = 1:T)
    # Capacity factor for electrolyzers is based on vUSE variable not vP
    if (!isempty(ELECTROLYZER))
        dfCapacityfactor.AnnualSum[ELECTROLYZER] .=
            value.(EP[:vUSE][ELECTROLYZER, :]).data * inputs["omega"] * scale_factor
        dfCapacityfactor.CapacityFactor[ELECTROLYZER] .=
            (
                dfCapacityfactor.AnnualSum[ELECTROLYZER] ./
