    ...
    if !isempty(MULTI_FUELS)
        ...
        for i in 1:max_fuels
            ...
            for g in MULTI_FUELS
                tempannualsum_fuel_heat_multi_generation[findfirst(x -> x == g, HAS_FUEL)] = value.(EP[:ePlantFuelConsumptionYear_multi_generation][
                    g, i])
                tempannualsum_fuel_heat_multi_start[findfirst(x -> x == g, HAS_FUEL)] = value.(EP[:ePlantFuelConsumptionYear_multi_start][
                    g, i])
                tempannualsum_fuel_heat_multi_total[findfirst(x -> x == g, HAS_FUEL)] = value.(EP[:ePlantFuelConsumptionYear_multi][
                    g, i])
                tempannualsum_fuel_cost_multi[findfirst(x -> x == g, HAS_FUEL)] = value.(EP[:ePlantCFuelOut_multi][
                    g, i]) +
                                                                                  value.(EP[:ePlantCFuelOut_multi_start][
                    g, i])
            end
