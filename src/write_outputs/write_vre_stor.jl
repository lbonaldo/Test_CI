...
function write_vre_stor_capacity(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    dfCap = DataFrame(
        Resource = inputs["RESOURCE_NAMES_VRE_STOR"],
        Zone = zones,
        Resource_Type = technologies,
        Cluster = clusters,
    ...
    total = DataFrame(
        ...
        NewCapWind = sum(dfCap[!, :NewCapWind]),
        EndCapWind = sum(dfCap[!, :EndCapWind]),
        StartCapDC = sum(dfCap[!, :StartCapDC]),
        RetCapDC = sum(dfCap[!, :RetCapDC]),
        NewCapDC = sum(dfCap[!, :NewCapDC]),
        EndCapDC = sum(dfCap[!, :EndCapDC]),
        StartCapGrid = sum(dfCap[!, :StartCapGrid]),
        RetCapGrid = sum(dfCap[!, :RetCapGrid]),
        NewCapGrid = sum(dfCap[!, :NewCapGrid]),
        EndCapGrid = sum(dfCap[!, :EndCapGrid]),
