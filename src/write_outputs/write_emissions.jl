...
function write_emissions(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if (
        setup["WriteShadowPrices"] == 1 ||
        setup["UCommit"] == 0 ||
        (
            setup["UCommit"] == 2 && (
                setup["OperationalReserves"] == 0 ||
                (setup["OperationalReserves"] > 0 && inputs["pDynamic_Contingency"] == 0)
            )
        )
    ) # fully linear model
        ...
        if setup["CO2Cap"] >= 1
            ...
            if has_duals(EP) == 1
                for cap = 1:inputs["NCO2Cap"]
                    for z in findall(x -> x == 1, inputs["dfCO2CapZones"][:, cap])
                        tempCO2Price[z, cap] =
                            (-1) * dual.(EP[:cCO2Emissions_systemwide])[cap]
            ...
            auxNew_Names = [
                Symbol("Zone")
                [Symbol("CO2_Price_$cap") for cap = 1:inputs["NCO2Cap"]]
                Symbol("AnnualSum")
            ]
        ...
        for i = 1:Z
            dfEmissions[i, :AnnualSum] =
                sum(inputs["omega"] .* emissions_by_zone[i, :]) * scale_factor
        end
        ...
        if setup["WriteOutputs"] == "annual"
            ...
            dfEmissions =
                hcat(dfEmissions, DataFrame(emissions_by_zone * scale_factor, :auto))
            if setup["CO2Cap"] >= 1
                auxNew_Names = [
                    Symbol("Zone")
                    [Symbol("CO2_Price_$cap") for cap = 1:inputs["NCO2Cap"]]
                ...
                rename!(dfEmissions, auxNew_Names)
                total = DataFrame(
                    ["Total" zeros(1, inputs["NCO2Cap"]) sum(dfEmissions[!, :AnnualSum]) fill(0.0, (1, T))],
                    :auto,
                )
                for t = 1:T
                    total[:, t+inputs["NCO2Cap"]+2] .=
                        sum(dfEmissions[:, Symbol("t$t")][1:Z])
                end
            else
                auxNew_Names =
                    [Symbol("Zone"); Symbol("AnnualSum"); [Symbol("t$t") for t = 1:T]]
                rename!(dfEmissions, auxNew_Names)
                total = DataFrame(
                    ["Total" sum(dfEmissions[!, :AnnualSum]) fill(0.0, (1, T))],
                    :auto,
                )
                for t = 1:T
        ...
        emissions_by_zone = value.(EP[:eEmissionsByZone])
        dfEmissions =
            hcat(DataFrame(Zone = 1:Z), DataFrame(AnnualSum = Array{Float64}(undef, Z)))
        for i = 1:Z
            dfEmissions[i, :AnnualSum] =
                sum(inputs["omega"] .* emissions_by_zone[i, :]) * scale_factor
        end
