...
function write_emissions(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if (
        ...
        if setup["WriteOutputs"] == "annual"
            ...
            dfEmissions = hcat(
                dfEmissions, DataFrame(emissions_by_zone * scale_factor, :auto))
            if setup["CO2Cap"] >= 1
                auxNew_Names = [Symbol("Zone")
                                [Symbol("CO2_Price_$cap") for cap in 1:inputs["NCO2Cap"]]
                                Symbol("AnnualSum")
                ...
                rename!(dfEmissions, auxNew_Names)
                total = DataFrame(
                    ["Total" zeros(1, inputs["NCO2Cap"]) sum(dfEmissions[!, :AnnualSum]) fill(0.0,
                        (1, T))],
                    :auto
                )
                for t in 1:T
                ...
                auxNew_Names = [Symbol("Zone"); Symbol("AnnualSum");
                                [Symbol("t$t") for t in 1:T]]
                rename!(dfEmissions, auxNew_Names)
                total = DataFrame(
        ...
        emissions_by_zone = value.(EP[:eEmissionsByZone])
        dfEmissions = hcat(
            DataFrame(Zone = 1:Z), DataFrame(AnnualSum = Array{Float64}(undef, Z)))
        for i in 1:Z
            dfEmissions[i, :AnnualSum] = sum(inputs["omega"] .* emissions_by_zone[i, :]) *
                                         scale_factor
        end
