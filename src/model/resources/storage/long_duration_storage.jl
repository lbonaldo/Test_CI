...
function long_duration_storage!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if CapacityReserveMargin > 0
        ...
        @variable(
            EP,
            vCAPRES_socw[y in STOR_LONG_DURATION, n in MODELED_PERIODS_INDEX] >= 0
        )
    ...
    @constraint(
        ...
        (1 - self_discharge(gen[y])) * (EP[:vS][y, hours_per_subperiod*w] - vdSOC[y, w]) -
    ...
    @constraint(
        ...
        vSOCw[y, mod1(r + 1, NPeriods)] ==
        vSOCw[y, r] + vdSOC[y, dfPeriodMap[r, :Rep_Period_Index]]
    )

    # Storage at beginning of each modeled period cannot exceed installed energy capacity
    @constraint(
        EP,
        cSoCBalLongDurationStorageUpper[
            y in STOR_LONG_DURATION,
    ...
    @constraint(
        ...
        vSOCw[y, r] ==
        EP[:vS][y, hours_per_subperiod*dfPeriodMap[r, :Rep_Period_Index]] -
    ...
    if CapacityReserveMargin > 0
        ...
        @constraint(
            ...
            EP[:vCAPRES_socinreserve][y, hours_per_subperiod*(w-1)+1] ==
            ...
            (efficiency_up(gen[y]) * EP[:vCAPRES_charge][y, hours_per_subperiod*(w-1)+1])
        )
        ...
        @constraint(
            EP,
            cVSoCBalLongDurationStorage[
                y in STOR_LONG_DURATION,
                r in MODELED_PERIODS_INDEX,
            ],
            vCAPRES_socw[y, mod1(r + 1, NPeriods)] ==
            vCAPRES_socw[y, r] + vCAPRES_dsoc[y, dfPeriodMap[r, :Rep_Period_Index]]
        )
        ...
        @constraint(
            EP,
            cSOCMinCapResLongDurationStorage[
                y in STOR_LONG_DURATION,
                r in MODELED_PERIODS_INDEX,
            ],
