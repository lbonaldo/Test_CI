function write_reserve_margin_slack(
        path::AbstractString,
        inputs::Dict,
        setup::Dict,
        EP::Model
    ...
    dfResMar_slack = DataFrame(
        CRM_Constraint = [Symbol("CapRes_$res") for res in 1:NCRM],
    ...
    if setup["WriteOutputs"] == "annual"
        ...
        dfResMar_slack = hcat(
            dfResMar_slack, DataFrame(temp_ResMar_slack, [Symbol("t$t") for t in 1:T]))
        CSV.write(
            joinpath(path, "ReserveMargin_prices_and_penalties.csv"),
