    ...
    for ParameterScale ∈ [0, 1]
            ...
            with_logger(warnerror_logger) do
                for t = 1:3
                    ...
                    true_min_retirements[t] = CSV.read(
                        joinpath(inpath_sub, "resources", "Resource_multistage_data.csv"),
                        DataFrame,
                    )
                    rename!(
                        true_min_retirements[t],
        ...
        for t = 1:3
            ...
            @test GenX.cum_min_retired_cap_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_mw for i = 1:t)
            @test GenX.cum_min_retired_energy_cap_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_energy_cap_mw for i = 1:t)
            @test GenX.cum_min_retired_charge_cap_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_charge_cap_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_inverter_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_inverter_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_solar_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_solar_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_wind_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_wind_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_discharge_dc_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_discharge_dc_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_charge_dc_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_charge_dc_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_discharge_ac_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_discharge_ac_mw for i = 1:t)
            @test GenX.cum_min_retired_cap_charge_ac_mw.(gen) ==
                  sum(true_min_retirements[i].min_retired_cap_charge_ac_mw for i = 1:t)
        end
    end
end
