...
function write_fuel_consumption_plant(
    path::AbstractString,
    inputs::Dict,
    setup::Dict,
    EP::Model,
    ...
    dfPlantFuel = DataFrame(
        Resource = inputs["RESOURCE_NAMES"][HAS_FUEL],
        Fuel = fuel.(gen[HAS_FUEL]),
        Zone = zone_id.(gen[HAS_FUEL]),
        AnnualSumCosts = zeros(length(HAS_FUEL)),
    )
    tempannualsum =
        value.(EP[:ePlantCFuelOut][HAS_FUEL]) + value.(EP[:ePlantCFuelStart][HAS_FUEL])
    ...
    if !isempty(MULTI_FUELS)
        ...
        for i = 1:max_fuels
            ...
            for g in MULTI_FUELS
                tempannualsum_fuel_heat_multi_generation[findfirst(x -> x == g, HAS_FUEL)] =
                    value.(EP[:ePlantFuelConsumptionYear_multi_generation][g, i])
                tempannualsum_fuel_heat_multi_start[findfirst(x -> x == g, HAS_FUEL)] =
                    value.(EP[:ePlantFuelConsumptionYear_multi_start][g, i])
                tempannualsum_fuel_heat_multi_total[findfirst(x -> x == g, HAS_FUEL)] =
                    value.(EP[:ePlantFuelConsumptionYear_multi][g, i])
                tempannualsum_fuel_cost_multi[findfirst(x -> x == g, HAS_FUEL)] =
                    value.(EP[:ePlantCFuelOut_multi][g, i]) +
                    value.(EP[:ePlantCFuelOut_multi_start][g, i])
            end
            ...
            dfPlantFuel[
                !,
                Symbol(
                    string(fuel_cols_num[i], "_AnnualSum_Fuel_HeatInput_Generation_MMBtu"),
                ),
            ] = tempannualsum_fuel_heat_multi_generation
            dfPlantFuel[
                !,
                Symbol(string(fuel_cols_num[i], "_AnnualSum_Fuel_HeatInput_Start_MMBtu")),
            ] = tempannualsum_fuel_heat_multi_start
            dfPlantFuel[
                !,
                Symbol(string(fuel_cols_num[i], "_AnnualSum_Fuel_HeatInput_Total_MMBtu")),
            ] = tempannualsum_fuel_heat_multi_total
            dfPlantFuel[!, Symbol(string(fuel_cols_num[i], "_AnnualSum_Fuel_Cost"))] =
                tempannualsum_fuel_cost_multi
        end
    end
...
function write_fuel_consumption_ts(
    path::AbstractString,
    inputs::Dict,
    setup::Dict,
    EP::Model,
    ...
    dfPlantFuel_TS = hcat(dfPlantFuel_TS, DataFrame(tempts, [Symbol("t$t") for t = 1:T]))
    CSV.write(
        joinpath(path, "FuelConsumption_plant_MMBTU.csv"),
        dftranspose(dfPlantFuel_TS, false),
        header = false,
    )
end


function write_fuel_consumption_tot(
    path::AbstractString,
    inputs::Dict,
    setup::Dict,
    EP::Model,
