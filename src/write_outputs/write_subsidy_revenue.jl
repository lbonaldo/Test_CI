    ...
    if (setup["MinCapReq"] >= 1)
        for mincap in 1:inputs["NumberOfMinCapReqs"] # This key only exists if MinCapReq >= 1, so we can't get it at the top outside of this condition.
            ...
            if !isempty(inputs["VRE_STOR"])
                ...
                MIN_CAP_GEN_SOLAR = ids_with_policy(
                    gen_VRE_STOR, min_cap_solar, tag = mincap)
                MIN_CAP_GEN_WIND = ids_with_policy(gen_VRE_STOR, min_cap_wind, tag = mincap)
                MIN_CAP_GEN_ASYM_DC_DIS = intersect(
                    inputs["VS_ASYM_DC_DISCHARGE"], HAS_MIN_CAP_STOR)
                MIN_CAP_GEN_ASYM_AC_DIS = intersect(
                    inputs["VS_ASYM_AC_DISCHARGE"], HAS_MIN_CAP_STOR)
