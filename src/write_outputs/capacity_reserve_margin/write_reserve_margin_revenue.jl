...
function write_reserve_margin_revenue(
    ...
    for i in 1:inputs["NCapacityReserveMargin"]
        weighted_price = capacity_reserve_margin_price(EP, inputs, setup, i) .*
                         inputs["omega"]
        tempresrev = zeros(G)
        tempresrev[THERM_ALL] = thermal_plant_effective_capacity(
            EP, inputs, THERM_ALL, i)' * weighted_price
        ...
        if !isempty(VRE_STOR)
            ...
            tempresrev[VRE_STOR_STOR] .-= derating_factor.(
                ...
                tag = i
            ) .* (value.(EP[:vCHARGE_VRE_STOR][
                VRE_STOR_STOR, :]).data * weighted_price)
            ...
            tempresrev[AC_DISCHARGE] .+= derating_factor.(
                ...
                tag = i
                                         ...
                                         ((value.(EP[:vCAPRES_AC_DISCHARGE][
                AC_DISCHARGE, :]).data) * weighted_price)
            ...
            tempresrev[AC_CHARGE] .-= derating_factor.(
                ...
                tag = i
            ) .* ((value.(EP[:vCAPRES_AC_CHARGE][AC_CHARGE, :]).data) *
                                       weighted_price)
        end
