function write_power_balance(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    Com_list = [
        ...
        "Storage_Discharge",
        "Storage_Charge",
        "Flexible_Demand_Defer",
        "Flexible_Demand_Stasify",
        "Demand_Response",
        "Nonserved_Energy",
        "Transmission_NetExport",
        "Transmission_Losses",
        "Demand",
    ]
    ...
    for z = 1:Z
        ...
        if !isempty(intersect(resources_in_zone_by_rid(gen, z), STOR_ALL))
            ...
            powerbalance[(z-1)*L+3, :] =
                (-1) * sum((value.(EP[:vCHARGE][STOR_ALL_ZONE, :]).data), dims = 1)
        end
        if !isempty(intersect(resources_in_zone_by_rid(gen, z), VRE_STOR))
            ...
            powerbalance[(z-1)*L+3, :] =
                (-1) * sum(value.(EP[:vCHARGE_VRE_STOR][VS_ALL_ZONE, :]).data, dims = 1)
        end
        if !isempty(intersect(resources_in_zone_by_rid(gen, z), FLEX))
            FLEX_ZONE = intersect(resources_in_zone_by_rid(gen, z), FLEX)
            powerbalance[(z-1)*L+4, :] =
                sum((value.(EP[:vCHARGE_FLEX][FLEX_ZONE, :]).data), dims = 1)
        ...
        if !isempty(ELECTROLYZER)
            ...
            powerbalance[(z-1)*L+11, :] =
                (-1) * sum(value.(EP[:vUSE][ELECTROLYZER_ZONE, :].data), dims = 1)
        end
    end
    ...
    if setup["WriteOutputs"] == "annual"
        ...
        CSV.write(
            joinpath(path, "power_balance.csv"),
