...
function storage_all!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(
        ...
        sum(inputs["omega"][t] * EP[:vCHARGE][y, t] for t = 1:T) -
        sum(inputs["omega"][t] * EP[:vP][y, t] for t = 1:T)
    )
    ...
    @expression(EP, eTotalCVarIn, sum(eTotalCVarInT[t] for t = 1:T))
    ...
    if CapacityReserveMargin > 0
        ...
        @expression(EP, eTotalCVarIn_virtual, sum(eTotalCVarInT_virtual[t] for t = 1:T))
        ...
        @expression(EP, eTotalCVarOut_virtual, sum(eTotalCVarOutT_virtual[t] for t = 1:T))
    ...
    @expression(
        EP,
        ePowerBalanceStor[t = 1:T, z = 1:Z],
        sum(
            EP[:vP][y, t] - EP[:vCHARGE][y, t] for
            y in intersect(resources_in_zone_by_rid(gen, z), STOR_ALL)
        )
    )
...
function storage_all_operational_reserves!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @constraint(
        EP,
        [y in STOR_REG, t in 1:T],
        efficiency_up(gen[y]) * (vCHARGE[y, t] + vREG_charge[y, t]) <=
        eTotalCapEnergy[y] - vS[y, hoursbefore(p, t, 1)]
    )
    # Note: maximum charge rate is also constrained by maximum charge power capacity, but as this differs by storage type,
    # this constraint is set in functions below for each storage type
