function write_capacity_value(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    for i in 1:inputs["NCapacityReserveMargin"]
        ...
        capvalue[riskyhour, THERM_ALL_EX] = thermal_plant_effective_capacity(
            EP, inputs, THERM_ALL_EX, i, riskyhour) ./
        ...
        if !isempty(VRE_STOR_EX)
            capres_dc_discharge = value.(EP[:vCAPRES_DC_DISCHARGE][
                DC_DISCHARGE, riskyhour].data)'
            ...
            charge_vre_stor = value.(EP[:vCHARGE_VRE_STOR][
                VRE_STOR_STOR_EX, riskyhour].data)'
            ...
            capvalue[riskyhour, DC_DISCHARGE_EX] += crm_derate(i, DC_DISCHARGE_EX) .*
                                                    capvalue_dc_discharge[
                riskyhour, DC_DISCHARGE_EX] ./
                                                    total_cap(DC_DISCHARGE_EX)
            capres_ac_discharge = value.(EP[:vCAPRES_AC_DISCHARGE][
                AC_DISCHARGE_EX, riskyhour].data)'
            ...
            capvalue[riskyhour, DC_CHARGE_EX] -= crm_derate(i, DC_CHARGE_EX) .*
                                                 capvalue_dc_charge[
                riskyhour, DC_CHARGE_EX] ./ total_cap(DC_CHARGE_EX)
