...
function generate_design_matrix(p_range, p_steps, rng; len_design_mat, groups)
    ...
    for j = 1:len_design_mat
        cur_p = [ps[u][(all_idxs[:, j][u])] for u = 1:length(p_range)]
        B[j] = cur_p
    end
    reduce(hcat, B)
end
...
function sample_matrices(
    ...
    if total_num_trajectory < num_trajectory
        error(
            "total_num_trajectory should be greater than num_trajectory preferably atleast 3-4 times higher",
        )
    end
    for i = 1:total_num_trajectory
...
function my_gsa(
    ...
    design_matrices_original = sample_matrices(
        p_range,
        p_steps,
        rng;
        num_trajectory,
    ...
    design_matrices =
        Matrix(DataFrame(unique(last, pairs(eachcol(L[!, 1:len_design_mat])))))
    distinct_trajectories[1] = length(design_matrices[1, :])
    if num_trajectory > 1
        for i = 2:num_trajectory
            design_matrices = hcat(
                design_matrices,
                Matrix(
                    DataFrame(
                        unique(
            ...
            distinct_trajectories[i] = length(
                Matrix(
                    DataFrame(
                        unique(
                            last,
    ...
    _y = [f(design_matrices[:, i]) for i = 1:size(design_matrices, 2)]
    ...
    if desol
        f_shape = x -> [reshape(x[:, i], y_size) for i = 1:size(x, 2)]
...
function morris(
    ...
    for column in uncertain_columns
        ...
        column_f =
            isdefined(GenX, col_sym) ? getfield(GenX, col_sym) :
            r -> getproperty(r, col_sym)
        sigma = [
            sigma
            [column_f.(gen) .* (
                1 .+
                Morris_range[Morris_range[!, :Parameter].==column, :Lower_bound] ./ 100
            ) column_f.(gen) .* (
                1 .+
                Morris_range[Morris_range[!, :Parameter].==column, :Upper_bound] ./ 100
            )]
        ]
    end
    sigma = sigma[2:end, :]
