...
function update_cumulative_min_ret!(
    inputs_d::Dict,
    t::Int,
    Resource_Set::String,
    RetCap::Symbol,
    ...
    ret_cap_f =
        isdefined(GenX, RetCap) ? getfield(GenX, RetCap) : r -> getproperty(r, RetCap)
    cum_ret_cap_f =
        isdefined(GenX, CumRetCap) ? getfield(GenX, CumRetCap) :
...
function endogenous_retirement!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if !isempty(inputs["VRE_STOR"])
        if !isempty(inputs["VS_DC"])
            endogenous_retirement_vre_stor_dc!(
                EP,
                inputs,
                num_stages,
                cur_stage,
        ...
        if !isempty(inputs["VS_SOLAR"])
            endogenous_retirement_vre_stor_solar!(
                EP,
                inputs,
                num_stages,
                cur_stage,
        ...
        if !isempty(inputs["VS_WIND"])
            endogenous_retirement_vre_stor_wind!(
                EP,
                inputs,
                num_stages,
                cur_stage,
        ...
        if !isempty(inputs["VS_STOR"])
            endogenous_retirement_vre_stor_stor!(
                EP,
                inputs,
                num_stages,
                cur_stage,
        ...
        if !isempty(inputs["VS_ASYM_DC_DISCHARGE"])
            endogenous_retirement_vre_stor_discharge_dc!(
                EP,
                inputs,
                num_stages,
                cur_stage,
        ...
        if !isempty(inputs["VS_ASYM_AC_DISCHARGE"])
            endogenous_retirement_vre_stor_discharge_ac!(
                EP,
                inputs,
                num_stages,
                cur_stage,
        ...
        if !isempty(inputs["VS_ASYM_AC_CHARGE"])
            endogenous_retirement_vre_stor_charge_ac!(
                EP,
                inputs,
                num_stages,
                cur_stage,
...
```
"""
function endogenous_retirement_discharge!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCap[y in RET_CAP], if y in NEW_CAP
        EP[:vCAP][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACK][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrack[y in RET_CAP],
        sum(
            EP[:vCAPTRACK][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
        EP,
        eMinRetCapTrack[y in RET_CAP],
        if y in COMMIT
...
function endogenous_retirement_charge!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapCharge[y in RET_CAP_CHARGE], if y in NEW_CAP_CHARGE
        EP[:vCAPCHARGE][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKCHARGE][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackCharge[y in RET_CAP_CHARGE],
        sum(
            EP[:vCAPTRACKCHARGE][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_energy!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapEnergy[y in RET_CAP_ENERGY], if y in NEW_CAP_ENERGY
        EP[:vCAPENERGY][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKENERGY][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackEnergy[y in RET_CAP_ENERGY],
        sum(
            EP[:vCAPTRACKENERGY][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_vre_stor_dc!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapDC[y in RET_CAP_DC], if y in NEW_CAP_DC
        EP[:vDCCAP][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKDC][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackDC[y in RET_CAP_DC],
        sum(
            EP[:vCAPTRACKDC][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_vre_stor_solar!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapSolar[y in RET_CAP_SOLAR], if y in NEW_CAP_SOLAR
        EP[:vSOLARCAP][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKSOLAR][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackSolar[y in RET_CAP_SOLAR],
        sum(
            EP[:vCAPTRACKSOLAR][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_vre_stor_wind!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapWind[y in RET_CAP_WIND], if y in NEW_CAP_WIND
        EP[:vWINDCAP][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKWIND][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackWind[y in RET_CAP_WIND],
        sum(
            EP[:vCAPTRACKWIND][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_vre_stor_stor!(
    ...
    @expression(EP, eNewCapEnergy_VS[y in RET_CAP_STOR], if y in NEW_CAP_STOR
        EP[:vCAPENERGY_VS][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKENERGY_VS][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackEnergy_VS[y in RET_CAP_STOR],
        sum(
            EP[:vCAPTRACKENERGY_VS][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_vre_stor_discharge_dc!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(
        EP,
        eNewCapDischargeDC[y in RET_CAP_DISCHARGE_DC],
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKDISCHARGEDC][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackDischargeDC[y in RET_CAP_DISCHARGE_DC],
        sum(
            EP[:vCAPTRACKDISCHARGEDC][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
    ...
    @constraint(
        ...
        eNewCapTrackDischargeDC[y] + eMinRetCapTrackDischargeDC[y] <=
        eRetCapTrackDischargeDC[y]
    )
end

function endogenous_retirement_vre_stor_charge_dc!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapChargeDC[y in RET_CAP_CHARGE_DC], if y in NEW_CAP_CHARGE_DC
        EP[:vCAPCHARGE_DC][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKCHARGEDC][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackChargeDC[y in RET_CAP_CHARGE_DC],
        sum(
            EP[:vCAPTRACKCHARGEDC][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
...
function endogenous_retirement_vre_stor_discharge_ac!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(
        EP,
        eNewCapDischargeAC[y in RET_CAP_DISCHARGE_AC],
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKDISCHARGEAC][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackDischargeAC[y in RET_CAP_DISCHARGE_AC],
        sum(
            EP[:vCAPTRACKDISCHARGEAC][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
    ...
    @constraint(
        ...
        eNewCapTrackDischargeAC[y] + eMinRetCapTrackDischargeAC[y] <=
        eRetCapTrackDischargeAC[y]
    )
end

function endogenous_retirement_vre_stor_charge_ac!(
    EP::Model,
    inputs::Dict,
    num_stages::Int,
    cur_stage::Int,
    ...
    @expression(EP, eNewCapChargeAC[y in RET_CAP_CHARGE_AC], if y in NEW_CAP_CHARGE_AC
        EP[:vCAPCHARGE_AC][y]
    else
        EP[:vZERO]
    end)
    ...
    @expression(
        ...
        sum(EP[:vRETCAPTRACKCHARGEAC][y, p] for p = 1:cur_stage)
    )
    @expression(
        EP,
        eNewCapTrackChargeAC[y in RET_CAP_CHARGE_AC],
        sum(
            EP[:vCAPTRACKCHARGEAC][y, p] for
            p = 1:get_retirement_stage(cur_stage, lifetime(gen[y]), stage_lens)
        )
    )
    @expression(
