...
"""
function write_reserve_margin_revenue(
    path::AbstractString,
    inputs::Dict,
    setup::Dict,
    EP::Model,
    ...
    for i = 1:inputs["NCapacityReserveMargin"]
        weighted_price =
            capacity_reserve_margin_price(EP, inputs, setup, i) .* inputs["omega"]
        tempresrev = zeros(G)
        tempresrev[THERM_ALL] =
            thermal_plant_effective_capacity(EP, inputs, THERM_ALL, i)' * weighted_price
        tempresrev[VRE] =
            derating_factor.(gen.Vre, tag = i) .* (value.(EP[:eTotalCap][VRE])) .*
            (inputs["pP_Max"][VRE, :] * weighted_price)
        tempresrev[MUST_RUN] =
            derating_factor.(gen.MustRun, tag = i) .* (value.(EP[:eTotalCap][MUST_RUN])) .*
            (inputs["pP_Max"][MUST_RUN, :] * weighted_price)
        tempresrev[HYDRO_RES] =
            derating_factor.(gen.Hydro, tag = i) .*
            (value.(EP[:vP][HYDRO_RES, :]) * weighted_price)
        if !isempty(STOR_ALL)
            tempresrev[STOR_ALL] =
                derating_factor.(gen.Storage, tag = i) .* (
                    (
        ...
        if !isempty(FLEX)
            tempresrev[FLEX] =
                derating_factor.(gen.FlexDemand, tag = i) .* (
                    (value.(EP[:vCHARGE_FLEX][FLEX, :]).data - value.(EP[:vP][FLEX, :])) *
                    weighted_price
                )
        end
        if !isempty(VRE_STOR)
            ...
            tempresrev[VRE_STOR] =
                derating_factor.(gen_VRE_STOR, tag = i) .*
                ((value.(EP[:vP][VRE_STOR, :])) * weighted_price)
            tempresrev[VRE_STOR_STOR] .-=
                derating_factor.(
