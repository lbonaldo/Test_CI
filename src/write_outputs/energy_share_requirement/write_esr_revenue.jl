        ...
        if !isempty(VRE_STOR)
            ...
            if !isempty(WIND_ONLY)
                ...
                dfESRRev[WIND, esr_col] =
                    (value.(EP[:vP_WIND][WIND, :]).data * weight) .*
                    esr_vrestor.(gen_VRE_STOR[wind_resources], tag = i) * price
            end
            if !isempty(SOLAR_WIND)
                solar_and_wind_resources =
                    ((gen_VRE_STOR.wind .!= 0) .& (gen_VRE_STOR.solar .!= 0))
                dfESRRev[SOLAR_WIND, esr_col] = (
                    (
                        (value.(EP[:vP_WIND][SOLAR_WIND, :]).data * weight) .*
                        esr_vrestor.(gen_VRE_STOR[solar_and_wind_resources], tag = i) *
                        price
                    ) +
                    (
                        value.(EP[:vP_SOLAR][SOLAR_WIND, :]).data .*
                        etainverter.(gen_VRE_STOR[solar_and_wind_resources]) * weight
                    ) .* esr_vrestor.(gen_VRE_STOR[solar_and_wind_resources], tag = i) *
                    price
                )
            end
