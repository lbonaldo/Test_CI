...
function fuel!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if !isempty(MULTI_FUELS)
        ...
        COFIRE_MAX = [
            findall(g -> max_cofire_cols(g, tag = i) < 1, gen[MULTI_FUELS]) for
            i = 1:max_fuels
        ]
        COFIRE_MAX_START = [
            findall(g -> max_cofire_start_cols(g, tag = i) < 1, gen[MULTI_FUELS]) for
            i = 1:max_fuels
        ]
        COFIRE_MIN = [
            findall(g -> min_cofire_cols(g, tag = i) > 0, gen[MULTI_FUELS]) for
            i = 1:max_fuels
        ]
        COFIRE_MIN_START = [
    ...
    @expression(
        EP,
        eStartFuel[y in 1:G, t = 1:T],
        if y in THERM_COMMIT
            (cap_size(gen[y]) * EP[:vSTART][y, t] * start_fuel_mmbtu_per_mw(gen[y]))
        else
            0
        end
    )
    ...
    @expression(
        EP,
        ePlantFuel_generation[y in 1:G, t = 1:T],
        if y in SINGLE_FUEL   # for single fuel plants
            EP[:vFuel][y, t]
        else # for multi fuel plants
            sum(EP[:vMulFuels][y, i, t] for i = 1:max_fuels)
        end
    )
    @expression(
        EP,
        ePlantFuel_start[y in 1:G, t = 1:T],
        if y in SINGLE_FUEL   # for single fuel plants
            EP[:vStartFuel][y, t]
        else # for multi fuel plants
            sum(EP[:vMulStartFuels][y, i, t] for i = 1:max_fuels)
        end
    )
    ...
    if !isempty(MULTI_FUELS)
        @expression(
            ...
            sum(omega[t] * EP[:vMulFuels][y, i, t] for t = 1:T)
        )
        @expression(
            EP,
            ePlantFuelConsumptionYear_multi_start[y in MULTI_FUELS, i in 1:max_fuels],
            sum(omega[t] * EP[:vMulStartFuels][y, i, t] for t = 1:T)
        )
        @expression(
            EP,
            ePlantFuelConsumptionYear_multi[y in MULTI_FUELS, i in 1:max_fuels],
            EP[:ePlantFuelConsumptionYear_multi_generation][y, i] +
            EP[:ePlantFuelConsumptionYear_multi_start][y, i]
        )
    end
    ...
    if !isempty(MULTI_FUELS)
        ...
        @expression(
            EP,
            eCFuelOut_multi_start[y in MULTI_FUELS, i in 1:max_fuels, t = 1:T],
            fuel_costs[fuel_cols(gen[y], tag = i)][t] * EP[:vMulStartFuels][y, i, t]
        )
        # annual plant level fuel cost by fuel type during generation
        @expression(
            EP,
            ePlantCFuelOut_multi_start[y in MULTI_FUELS, i in 1:max_fuels],
            sum(omega[t] * EP[:eCFuelOut_multi_start][y, i, t] for t = 1:T)
        )
    ...
    @expression(
        EP,
        eCFuelStart[y = 1:G, t = 1:T],
        if y in SINGLE_FUEL
            (fuel_costs[fuel(gen[y])][t] * EP[:vStartFuel][y, t])
        else
            sum(EP[:eCFuelOut_multi_start][y, i, t] for i = 1:max_fuels)
        end
    )
    ...
    @expression(
        EP,
        ePlantCFuelStart[y = 1:G],
        sum(omega[t] * EP[:eCFuelStart][y, t] for t = 1:T)
    )
    # zonal level total fuel cost for output
    @expression(
        EP,
        eZonalCFuelStart[z = 1:Z],
    ...
    if !isempty(MULTI_FUELS)
        ...
        @expression(
            EP,
            eCFuelOut_multi[y in MULTI_FUELS, i in 1:max_fuels, t = 1:T],
            fuel_costs[fuel_cols(gen[y], tag = i)][t] * EP[:vMulFuels][y, i, t]
        )
        # annual plant level fuel cost by fuel type during generation
        @expression(
            EP,
            ePlantCFuelOut_multi[y in MULTI_FUELS, i in 1:max_fuels],
            sum(omega[t] * EP[:eCFuelOut_multi][y, i, t] for t = 1:T)
        )
    ...
    @expression(
        EP,
        eCFuelOut[y = 1:G, t = 1:T],
        if y in SINGLE_FUEL
            (fuel_costs[fuel(gen[y])][t] * EP[:vFuel][y, t])
        else
            sum(EP[:eCFuelOut_multi][y, i, t] for i = 1:max_fuels)
        end
    )
    # plant level start-up fuel cost for output
    @expression(
        EP,
        ePlantCFuelOut[y = 1:G],
        sum(omega[t] * EP[:eCFuelOut][y, t] for t = 1:T)
    )
    # zonal level total fuel cost for output
    @expression(
        EP,
        eZonalCFuelOut[z = 1:Z],
        sum(EP[:ePlantCFuelOut][y] for y in resources_in_zone_by_rid(gen, z))
    )


    # system level total fuel cost for output
    @expression(EP, eTotalCFuelOut, sum(eZonalCFuelOut[z] for z = 1:Z))
    @expression(EP, eTotalCFuelStart, sum(eZonalCFuelStart[z] for z = 1:Z))
    ...
    if !isempty(MULTI_FUELS)
        @expression(
            EP,
            eFuelConsumption_multi[f in 1:NUM_FUEL, t in 1:T],
            sum(
                (EP[:vMulFuels][y, i, t] + EP[:vMulStartFuels][y, i, t]) #i: fuel id 
                for i = 1:max_fuels, y in intersect(
    ...
    @expression(
        EP,
        eFuelConsumption_single[f in 1:NUM_FUEL, t in 1:T],
        sum(
            EP[:vFuel][y, t] + EP[:eStartFuel][y, t] for
            y in intersect(resources_with_fuel(gen, fuels[f]), SINGLE_FUEL)
        )
    )

    @expression(
        EP,
        eFuelConsumption[f in 1:NUM_FUEL, t in 1:T],
        if !isempty(MULTI_FUELS)
    ...
    @expression(
        EP,
        eFuelConsumptionYear[f in 1:NUM_FUEL],
        sum(omega[t] * EP[:eFuelConsumption][f, t] for t = 1:T)
    )
    ...
    if !isempty(MULTI_FUELS)
        @constraint(
            ...
            cFuelCalculation_multi[
                y in intersect(MULTI_FUELS, setdiff(HAS_FUEL, THERM_COMMIT)),
                t = 1:T,
            ],
            sum(EP[:vMulFuels][y, i, t] / heat_rates[i][y] for i = 1:max_fuels) -
            EP[:vP][y, t] == 0
        )
    end
    ...
    if !isempty(THERM_COMMIT)
        ...
        @constraint(
            ...
            FuelCalculationCommit_single[
                y in intersect(setdiff(THERM_COMMIT, THERM_COMMIT_PWFU), SINGLE_FUEL),
                t = 1:T,
            ],
            EP[:vFuel][y, t] - EP[:vP][y, t] * heat_rate_mmbtu_per_mwh(gen[y]) == 0
        )
        if !isempty(MULTI_FUELS)
            @constraint(
                ...
                FuelCalculationCommit_multi[
                    y in intersect(setdiff(THERM_COMMIT, THERM_COMMIT_PWFU), MULTI_FUELS),
                    t = 1:T,
                ],
                sum(EP[:vMulFuels][y, i, t] / heat_rates[i][y] for i = 1:max_fuels) -
                EP[:vP][y, t] .== 0
    ...
    @constraint(
        EP,
        cStartFuel_single[y in intersect(THERM_COMMIT, SINGLE_FUEL), t = 1:T],
        EP[:vStartFuel][y, t] -
    ...
    if !isempty(MULTI_FUELS)
        for i = 1:max_fuels
            ...
            @constraint(
                EP,
                [y in intersect(MULTI_FUELS, COFIRE_MIN[i]), t = 1:T],
                EP[:vMulFuels][y, i, t] >=
                min_cofire[i][y] * EP[:ePlantFuel_generation][y, t]
            )
            @constraint(
                EP,
                [y in intersect(MULTI_FUELS, COFIRE_MAX[i]), t = 1:T],
                EP[:vMulFuels][y, i, t] <=
                max_cofire[i][y] * EP[:ePlantFuel_generation][y, t]
            )
            # startup
            @constraint(
                EP,
                [y in intersect(MULTI_FUELS, COFIRE_MIN_START[i]), t = 1:T],
                EP[:vMulStartFuels][y, i, t] >=
                min_cofire_start[i][y] * EP[:ePlantFuel_start][y, t]
            )
            @constraint(
                EP,
                [y in intersect(MULTI_FUELS, COFIRE_MAX_START[i]), t = 1:T],
                EP[:vMulStartFuels][y, i, t] <=
                max_cofire_start[i][y] * EP[:ePlantFuel_start][y, t]
            )
        end
    end
